...
-- 1. Registrar exclusão de usuários. Crie uma trigger que insira os dados do usuário excluído na tabela UsuarioExcluido.
-- Essa é a tabela que vc cria com as informações que vc quer saber do usuario excluido
...
-- criei um usuario, nn precisa, mas fiz porq os outros tem emprestimos e iria ser mais dificil
INSERT INTO Usuario (nome, numero_identificacao, email) VALUES ('Kamila Vitoria', '03948', 'kamilavsc@email.com');
-- vc deleta o usuario
-- o trigger vai ver essa situação e vai salvar no historico que na tabela usuario um dado foi apagado e vai salvar em usuarioexcluido
DELIMITER //
...
-- selecionar as tabelas para ver a atualização
SELECT * FROM Usuario;
...
-- 2. Preencher data de empréstimo se for omitida. Crie uma trigger que preenche data_emprestimo com a data atual se ela vier nula.
...
CREATE TABLE emprestimo_historico(
    ...
    livro INT,
    data_emprestimo DATE,
    data_devolucao_ant DATE,
    data_devolucao_nv DATE
);
DELIMITER //
...
INSERT Emprestimo (USUARIO_ID, LIVRO_ID, DATA_EMPRESTIMO, DATA_DEVOLUCAO_LIMITE, DATA_DEVOLUCAO) VALUES (410, 306, NULL, CURDATE(), CURDATE());
SELECT * FROM Emprestimo;
SELECT * FROM emprestimo_historico;


-- 3. Registrar alteração de nome de livro. Crie uma trigger para registrar toda vez que o nome de um livro for alterado.
CREATE TABLE livros_historico(
    id_livro INT,
    isbn_livro VARCHAR(20),
    nome_antigo VARCHAR(200),
    nome_Atual VARCHAR(200)
);
...
BEGIN
    ...
    INSERT INTO livros_historico (id_livro, isbn_livro, nome_antigo, nome_Atual)
...
UPDATE Livro SET titulo = "Crepúsculo" WHERE id = 301;

SELECT * FROM Livro;
SELECT * FROM livros_historico;

-- 4. Salvar log de cadastro de usuários. Crie uma trigger que registre todo novo usuário em uma tabela de log.
...
FOR EACH ROW
BEGIN
	INSERT INTO usuario_log (usuario_id, nome, numero_identificacao, email, data_cadastro, nivel_associacao)
    VALUES (NEW.id, NEW.nome, NEW.numero_identificacao, NEW.email, NEW.data_cadastro, NEW.nivel_associacao);
END;
//
DELIMITER ;

INSERT INTO Usuario (nome, numero_identificacao, email, data_cadastro, nivel_associacao)
VALUES ('Kamila Cardozo', '10ka', 'kamilalinda@gmail.com', '2025-06-01', 'regular');
SELECT * FROM usuario_log;
SELECT * FROM Usuario;


-- 5. Log ao adicionar livro com ISBN. Crie uma trigger que grave o título e o ISBN de todo novo livro inserido.
CREATE TABLE livros_novos(
	titulo VARCHAR(200),
    isbn VARCHAR(20)
    
);
-- DROP TRIGGER trg_livro;
DELIMITER //
CREATE TRIGGER trg_livro
AFTER INSERT ON Livro
FOR EACH ROW
BEGIN
	INSERT INTO livros_novos (titulo, isbn)
...
VALUES ('Eclipse', '078177892', 'Esse livro faz parte da saga do crepusculo, um mundo de humanos, vampiros e lobisomens.');
SELECT * FROM livros_novos;
SELECT * FROM Livro;